from board import create_board, print_board, valid_shot_coord, cpu_choose_shot
from ships import deploy_fleet, process_shot
...
def main(): #setup/initialize game
    ...
    while player_remaining > 0 and cpu_remaining > 0:
        ...
        result = process_shot(shot, cpu_board, cpu_lookup)
        if result == "Sunk":
            cpu_remaining -= 1
        ...
        # CPU's turn
        cpu_shot = cpu_choose_shot()
        print(f"CPU fired at {chr(65 + cpu_shot[0])}{cpu_shot[1] + 1}")
        result = process_shot(cpu_shot, player_board, player_lookup)
        if result == "Sunk":
            player_remaining -= 1
